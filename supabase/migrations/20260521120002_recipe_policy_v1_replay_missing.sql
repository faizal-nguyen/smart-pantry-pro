-- =========================================================================
-- PRP-239 PR1b/4-fix — Replay missing manifest changes.
--
-- BUG fixed: 20260521120001 used `sanitization_versions[<domain>.v1]` as a
-- per-domain idempotency early-skip. After the first pork.* (or alcohol.*)
-- change landed on a recipe, every subsequent change of the same domain
-- on that same recipe was skipped with `RAISE NOTICE 'already_applied'`.
-- Recipes with multiple changes in the same domain (e.g. Kimchi Jjigae:
-- porc belly in name + porc gras in notes) ended up partially corrected.
--
-- Symptom verified on:
--   SELECT ingredient_name, notes
--     FROM recipe_ingredients ri
--     JOIN recipes r ON r.id = ri.recipe_id
--    WHERE r.name = 'Kimchi Jjigae (Ragoût de Kimchi)';
--   → ingredient_name='boeuf gras', notes='ou porc gras'  -- notes still has porc!
--
-- This migration replays the manifest with **per-change idempotency** :
-- it looks for any ingredient whose target field STILL contains the
-- manifest's old_value (via `_policy_substitute(...) <> field`). If
-- found → apply. If not → silently continue (either re-run or genuine
-- miss, both fine here — the v1 manifest is closed).
--
-- Net effect: any change skipped by the bug in 20260521120001 now lands.
-- Re-running this migration is a no-op (substitutions become identity).
-- =========================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- 1. Resolve recipe (must be unique).
    ...
    IF v_match_count <> 1 THEN
      -- 0 = recipe not in DB (already logged by initial run if applicable).
      -- >1 = ambiguous (already logged). Skip silently here to avoid log churn.
      CONTINUE;
    END IF;
    ...
    -- 2. Find an ingredient row whose target field STILL contains the
    -- bounded token old_value. If found, it needs substitution.
    v_ingredient_id := NULL;
    v_ingredient_value := NULL;
    ...
    IF v_ingredient_id IS NULL THEN
      -- Either already applied in the initial run (good) OR no longer
      -- relevant. Either way, nothing to do here.
      CONTINUE;
    END IF;
    ...
    -- 4. Log to recipe_corrections_log.
    ...
    VALUES
      (v_recipe_id, v_field, v_ingredient_value, v_new_field_value, v_rule_id,
       v_quality_flag, v_manifest_version, 'prp-239-pr1b-replay')
    ON CONFLICT DO NOTHING;

    -- 5. Merge quality_flag + sanitization_versions on the recipe.
  ...
  RAISE NOTICE 'PRP-239 PR1b replay complete. Additional changes applied: %', v_applied_count;
END;
$migration$;
