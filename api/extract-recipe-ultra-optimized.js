...
import { extractRecipeWithPuppeteer } from './extract-recipe-spa.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isProblematicSite) {
      console.log('⚠️ Site SPA détecté, utilisation de Puppeteer:', url);
      
      try {
        const startTime = Date.now();
        console.log('🎭 Démarrage extraction Puppeteer pour:', url);
        
        const recipe = await extractRecipeWithPuppeteer(url);
        console.log('📊 Recette extraite:', recipe.name, 'avec', recipe.ingredients.length, 'ingrédients');
        
        // Valider que la recette a du contenu
        if (!recipe.name || recipe.name === 'Recette sans nom' || recipe.ingredients.length === 0) {
          console.error('⚠️ Recette vide ou invalide');
          throw new Error('Extraction incomplète');
        }
        ...
        return res.status(200).json({
          ...
          performance: {
            ...
            model_used: 'puppeteer',
            content_size: 0,
            structured_data: false
          }
        });
      } catch (puppeteerError) {
        console.error('❌ Échec extraction Puppeteer:', puppeteerError.message);
        console.error('Stack:', puppeteerError.stack);
        return res.status(400).json({
          success: false,
          error: 'Extraction impossible. Essayez de copier-coller la recette manuellement.',
          details: process.env.NODE_ENV === 'development' ? puppeteerError.message : undefined
        });
      }
    }
