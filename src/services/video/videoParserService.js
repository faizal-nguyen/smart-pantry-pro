...
class VideoParserService {
  ...
  async getInstagramThumbnail(videoUrl) {
    this.addLog('🖼️ [VideoParserService] Getting Instagram thumbnail...');
    
    try {
      const pythonScript = path.join(__dirname, 'instagram_metadata.py');
      
      return new Promise((resolve, reject) => {
        const pythonProcess = spawn('python3', [pythonScript, videoUrl]);
        ...
        pythonProcess.on('close', (code) => {
          if (code !== 0) {
            this.addLog('❌ [VideoParserService] Python script failed:', error);
            resolve(null); // Ne pas faire échouer l'extraction si la vignette ne fonctionne pas
            return;
          }
          ...
          try {
            ...
            if (result.success) {
              this.addLog('✅ [VideoParserService] Thumbnail extracted successfully');
              resolve({
              ...
              this.addLog('⚠️ [VideoParserService] Thumbnail extraction failed:', result.error);
              resolve(null);
            }
          } catch (parseError) {
            this.addLog('❌ [VideoParserService] Failed to parse thumbnail response:', parseError.message);
            resolve(null);
          }
        });

        pythonProcess.on('error', (err) => {
          this.addLog('❌ [VideoParserService] Failed to start Python process:', err.message);
          resolve(null);
        });
      });
    } catch (error) {
      this.addLog('❌ [VideoParserService] Thumbnail extraction error:', error.message);
      return null;
    }
  }
  ...
  async parseInstagramReel(videoUrl) {
    ...
    // Récupérer la vignette en parallèle de l'extraction principale
    const thumbnailPromise = this.getInstagramThumbnail(videoUrl);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Récupérer la vignette même en mode démo
      ...
      // Étape 6: Récupérer la vignette
      this.addLog('🖼️ [VideoParserService] Step 6: Getting thumbnail...');
      const thumbnailData = await thumbnailPromise;
